<div class="max-w-[1320px] mx-auto px-5 sm:px-8 py-8">
    ...
    <div class="paper-card rounded-4xl p-6 sm:p-8 shadow-card border border-cream-300/50 mt-4 relative overflow-hidden">
        ...
        <div class="relative">
            ...
            <h1 class="font-modern text-[44px] sm:text-[56px] leading-[0.95] text-ink-900 mt-1">{{ $shelter->name }}</h1>
            ...
            <div class="flex flex-wrap gap-3 mt-5">
                <div class="paper-note rounded-2xl px-4 py-2.5">
                    <div class="font-modern text-[22px] text-clay-600">{{ $animals->count() }}</div>
                    <div class="text-[11px] text-ink-700/65 uppercase tracking-wider">yayında dost</div>
                </div>
                <div class="paper-note rounded-2xl px-4 py-2.5">
                    <div class="font-modern text-[22px] text-clay-600">₺{{ number_format($totalRaised, 0, ',', '.') }}</div>
    ...
    @if($announcements->isNotEmpty())
        <div class="mt-7">
            <h2 class="font-modern text-[26px] text-ink-900 mb-3">Barınaktan haberler</h2>
            <div class="grid sm:grid-cols-2 gap-4">
                @foreach($announcements as $a)
                    <div class="paper-note rounded-2xl p-4 shadow-note note-tilt-1">
                        <div class="font-modern text-[17px] text-ink-900">{{ $a->title }}</div>
    ...
    {{-- Hayvanlar --}}
    <div class="mt-8">
        <h2 class="font-modern text-[30px] text-ink-900 mb-4">Buradaki dostlar</h2>
        @if($animals->isEmpty())
            <div class="paper-card rounded-4xl border border-cream-300/50 p-10 text-center font-modern text-[22px] text-clay-500">
