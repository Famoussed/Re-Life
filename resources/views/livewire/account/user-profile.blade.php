<div class="max-w-[1000px] mx-auto px-5 sm:px-8 py-8">
    ...
    <div class="paper-card rounded-4xl p-6 sm:p-8 shadow-card border border-cream-300/50 relative overflow-hidden">
        ...
        <div class="relative flex items-center gap-5 flex-wrap">
            <div class="w-20 h-20 rounded-full bg-sage-200 flex items-center justify-center font-serif text-[36px] text-sage-700">
        ...
        <div class="flex flex-wrap gap-3 mt-6">
            <div class="paper-note rounded-2xl px-5 py-3">
                <div class="font-serif text-[26px] text-clay-600">₺{{ number_format((float) $user->total_donated, 0, ',', '.') }}</div>
                <div class="text-[11px] text-ink-700/65 uppercase tracking-wider">toplam bağış</div>
            </div>
            <div class="paper-note rounded-2xl px-5 py-3">
                <div class="font-serif text-[26px] text-clay-600">{{ $donationCount }}</div>
                <div class="text-[11px] text-ink-700/65 uppercase tracking-wider">bağış adımı</div>
            </div>
            <div class="paper-note rounded-2xl px-5 py-3">
                <div class="font-serif text-[26px] text-clay-600">Sv. {{ $user->badge_level }}</div>
    ...
    <div class="grid lg:grid-cols-12 gap-7 mt-7">
        ...
        <div class="lg:col-span-7">
            <h2 class="font-serif text-[26px] text-ink-900 mb-3">Bağış geçmişi</h2>
                ...
                @forelse($donations as $i => $d)
                    <div class="flex items-center gap-4 px-5 py-3.5 {{ $i > 0 ? 'border-t border-dashed border-clay-200' : '' }}">
                        ...
                        <div class="text-right">
                            <div class="font-serif text-[17px] text-ink-900">₺{{ number_format((float) $d->amount, 0, ',', '.') }}</div>
                            <div class="text-[11px] text-ink-700/50">{{ $d->created_at->format('d.m.Y') }}</div>
                        </div>
                    </div>
                @empty
                    <div class="p-10 text-center font-hand text-[22px] text-clay-500">henüz bir adım atılmadı</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="lg:col-span-5">
            <h2 class="font-serif text-[26px] text-ink-900 mb-3">Desteklediği dostlar</h2>
            <div class="paper-card rounded-4xl shadow-card border border-cream-300/50 p-5">
                @if($supportedAnimals->isEmpty())
                    <div class="text-center font-hand text-[20px] text-clay-500 py-6">henüz yok</div>
                @else
