<div class="max-w-[640px] mx-auto px-5 sm:px-8 py-10">
    ...
    @if($step === 3 && $done)
        ...
        <div class="paper-card rounded-4xl p-8 shadow-card border border-cream-300/50 text-center">
            ...
            <h1 class="font-serif text-[40px] text-ink-900 leading-tight mt-2">Bir adım daha atıldı</h1>
    ...
    <h1 class="font-serif text-[42px] text-ink-900 leading-tight mt-2">Bir adım hediye et</h1>
    ...
    <div class="paper-note rounded-2xl p-4 shadow-note mt-4">
        @if($selectedNeed)
            ...
            <div class="font-serif text-[19px] text-ink-900 mt-0.5">{{ $selectedNeed->title }}</div>
        ...
        @elseif($selectedShelter)
            ...
            <div class="font-serif text-[19px] text-ink-900 mt-0.5">{{ $selectedShelter->name }}</div>
            ...
            <div class="grid grid-cols-4 gap-2">
                @foreach([50, 100, 250, 500] as $preset)
                    <button type="button" wire:click="pickAmount({{ $preset }})"
                        ...
                        <div class="font-serif text-[20px] text-clay-600">₺{{ $preset }}</div>
                    </button>
                @endforeach
            </div>
            <div class="mt-3">
    ...
    @if($step === 2)
        ...
        <div class="paper-card rounded-4xl p-6 sm:p-7 shadow-card border border-cream-300/50 mt-5">
            <div class="flex items-baseline justify-between">
                ...
                <div class="font-serif text-[22px] text-ink-900">₺{{ number_format($amount, 0, ',', '.') }}</div>
            </div>
