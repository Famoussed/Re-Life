<div>
    <header class="mb-7">
        <h1 class="font-modern text-[34px] leading-tight text-ink-900">Bağışçılar</h1>
    ...
    @if($donors->isEmpty())
        <div class="paper-card rounded-4xl border border-cream-300/50 p-12 text-center">
            <div class="font-modern text-[24px] text-clay-500">henüz bağışçınız yok</div>
        </div>
    @else
                ...
                <tbody class="divide-y divide-cream-300/40">
                    @foreach($donors as $i => $donor)
                        <tr wire:key="donor-{{ $donor['user']->id }}" class="hover:bg-cream-100/50">
                            <td class="px-5 py-3 font-modern text-clay-500">{{ $i + 1 }}</td>
